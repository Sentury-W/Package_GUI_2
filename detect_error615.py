...
import numpy as np
import pandas as pd
from error_detect import detect_st_threshold, fill_point_B_spline1, fill_point
from error_detect import three_sigma, detect_st_threshold, box_plot, iostation_forest, k_means, svm_
...
model = 1     # 模式选择  1-单索力监测  2-某月的所有索力结果汇集在一张图
method = 1    # 五种方法 按照上面的列表选择1~5
month = 2     # 进行月份的选择
id = 'SLX22'  # 索力选择 用于监测某月 某一个索力数据  【model=2时 该参数无用】
...
if __name__ == "__main__":
    ...
    if model == 1:
        ...
        if method == 1:
            ...
            ts = three_sigma()
            # error_index_list = JY_error(SLdata_list, MAXDATA=MAX_data[j], MINDATA=MIN_data[j])
            error_index_list2 = detect_st_threshold(SLdata_list, sensor_id)
            new_list1 = fill_point(SLdata_list, error_index_list2)
            new_list2 = fill_point_B_spline1(SLdata_list, error_index_list2)
            # error_index_list2 = ts.three_sigma2_(new_list2, np.mean(new_list2), np.std(new_list2), MAX=MAX_data[j],
            #                                      MIN=MIN_data[j])
        ...
        ax = fg.add_subplot(3, 1, 2)
        ax.plot(SLMdate_list, new_list1)
        ax.set_xlabel("Date")
        ax.set_ylabel("SLDate")
        ax.set_title('插值填补后' + sensor_id + ' ' + METHOD[method - 1])
        # for error_index in error_index_list2:
        #     ax.scatter(SLMdate_list[error_index], SLdata_list[error_index], c="r")
        # fg.savefig('插值填补后' + str(i) + "-" + sensor_id + ' ' + METHOD[method - 1] + '.jpg')

        ax = fg.add_subplot(3, 1, 3)
        ax.plot(SLMdate_list, new_list2)
        ax.set_xlabel("Date")
        ax.set_ylabel("SLDate")
        ax.set_title('B-spline填补后' + sensor_id + ' ' + METHOD[method - 1])
        # for error_index in error_index_list2:
        #     ax.scatter(SLMdate_list[error_index], SLdata_list[error_index], c="r")
        fg.tight_layout()
        fg.savefig(str(i) + "-" + sensor_id + ' ' + METHOD[method - 1] + '.jpg')
        ...
        for sensor_id in SENSOR_ID_LIST:
            ...
            if method == 1:
                ...
                # ts = three_sigma()  # 修改这里的经验值判断为threshold那个函数使用
            ...
            for error_index in error_index_list2:
                ax.scatter(SLMdate_list[error_index], SLdata_list[error_index], c="r")
        fg.tight_layout()
        fg.savefig('2021-' + str(i) + '月 ' + METHOD[method - 1] + '.jpg')
